...
function openCamera() {
  ...
  if (!modal || !video) {
    alert("Camera interface not found. Please refresh the page and try again.")
    return
  }
...
function capturePhoto() {
  ...
  if (!video || !canvas || !preview || !previewContainer || !fileInput || !uploadStatus) {
    ...
    alert("Camera interface elements missing. Please refresh the page.")
    return
  }
    ...
    (blob) => {
      ...
      preview.onload = () => {
        ...
        try {
          ...
          uploadStatus.innerHTML = `<span class="success">✅ Photo captured successfully (${fileSizeKB} KB)</span>`
          
          console.log("Photo captured and assigned to input:", fileName, fileSizeKB + "KB")
          ...
          uploadStatus.innerHTML = '<span class="error">❌ Error processing captured image</span>'
        }
